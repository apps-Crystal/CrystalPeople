...
"[project]/src/app/api/monthly/score-team/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(req) {
    ...
    if (session.role === "manager") {
        team = allEmployees.filter((e)=>e.Manager_ID === session.userId && e.Status === "active" && e.Employee_ID !== session.userId);
    } else {
    ...
    if (employeeId) {
        ...
        const empGoals = goals.filter((g)=>g.Employee_ID === employeeId && g.Month === month && g.Year === year);
        const empTasks = tasks.filter((t)=>t.Employee_ID === employeeId);
        ...
        const cycle = cycles.find((c)=>c.Employee_ID === employeeId && c.Month === month && c.Year === year) ?? null;
        ...
        if (cycle) {
            ...
            const managerScores = (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$utils$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["safeJsonParse"])(cycle.Manager_Scores, {});
    ...
    const teamData = team.map((employee)=>{
        const empGoals = goals.filter((g)=>g.Employee_ID === employee.Employee_ID && g.Month === month && g.Year === year);
        const empTasks = tasks.filter((t)=>t.Employee_ID === employee.Employee_ID);
        const dimensions = (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$utils$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getScoreDimensions"])(employee, empGoals, empTasks);
        const cycle = cycles.find((c)=>c.Employee_ID === employee.Employee_ID && c.Month === month && c.Year === year) ?? null;
...
//# sourceMappingURL=%5Broot-of-the-server%5D__21058d69._.js.map